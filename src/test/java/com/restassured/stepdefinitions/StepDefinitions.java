...
public class StepDefinitions{
	...
	public void issue_payload_with(String title, String description, String labels){
		requestBody = given().spec(Utils.requestSpecifications()).body(data.newIssuePayload(title, description, labels));
	}
	...
	public void in_the_response_body_is(String keyValue, String Expectedvalue) {
		assertEquals(Utils.getJsonPath(responseBody, keyValue), Expectedvalue);
	}
	...
	public void verify_iid_maps_to_using(String expectedTitle, String resource) throws FileNotFoundException {
		...
		iid = Utils.getJsonPath(responseBody, "iid");
		requestBody = given().spec(Utils.requestSpecifications()).pathParam("iid", iid);
		user_calls_with_http_request(resource, "GET");
		// String actualTitle = getJsonPath(responseBody, "title");
		assertEquals(Utils.getJsonPath(responseBody, "title"), expectedTitle);
	}
	...
	public void issue_payload_without_private_token(String title, String description, String labels)
		...
		requestBody = given().spec(Utils.requestSpecificationsWithoutToken())
	...
	public void delete_issue_payload_without_private_token() throws FileNotFoundException {
		requestBody = given().spec(Utils.requestSpecificationsWithoutToken()).pathParam("iid", iid);
	}

	@Given("^Get Issue Payload with invalid (.*)")
	public void get_issue_payload_with_invalid_issue_id(String iid) throws FileNotFoundException {
		//String iid = "abc123";
		requestBody = given().spec(Utils.requestSpecifications()).pathParam("iid", iid);
	}
}