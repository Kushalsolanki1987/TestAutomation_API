...
|ApplicationIssue123      |Preprod1 ENV is DOWN|SERVER1     |
  ...
  @Regression
 Scenario Outline: Verify if the user is able to edit issue using Edit Issue API
...
And "message" in the response body is "404 Not found"
  ...
  @Regression
 Scenario Outline: Verify if  New Issue API throws an error when accessed without private token.
Given Issue Payload with "<title>" "<description>" "<labels>" without Private Token.
 ...
 @DeleteIssue
 Scenario: Verify if the Delete Issue API throws an error when accessed without private token.
 ...
 Scenario: Verify if the Get Issue API throws an error when called with invalid Issue ID
